people = {"nile": "eygpt", "Amazon":"brazil", "Ganges":"india"}
for index in people:
    print(f"the river {index} runs through {people[index]}")


dogs = {"john": "dog","lbozo": "ugly dog"}
cats = {"hit;er": "egyptian cat (ew)", "old women": "1000 cats"}
pets = [dogs,cats]
for pet in pets:
    for animal in pet:
        print(f" {animal} owns a {pet[animal]}")
...
meanAge = totalAge/len(students)
commenage = 0
for index in ages:
    if (ages.count(index)>commenage):
        commenage = ages.count(index)
...
print(f"The lowest age is {youngest[0]['age']} and it is {youngest[0]['given_name']} {youngest[0]['family_name']}")
print(f"The mean age of the student body is {meanAge:.2f}")
print(f"mode age is {modeage}")
